...
import numpy as np
from torchvision.models import alexnet  # 导入 AlexNet
import csv
...
def random_hyperparams():
    lr = random.choice([1e-4, 5e-5, 5e-4])
    weight_decay = random.choice([1e-3, 1e-4, 1e-2])
    batch_size = random.choice([32, 64])
    return lr, weight_decay, batch_size

def main():
    model_name = "AlexNet"
    dataset_root = '金文手写+金文拓片转手写diffusion'
    DateFrom = '金文手写+金文拓片转手写diffusion_增广vit'
