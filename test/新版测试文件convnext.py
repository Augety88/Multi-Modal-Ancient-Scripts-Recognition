...
import numpy as np
from torchvision.models import convnext_tiny  # 替换为ConvNeXt Tiny
import csv
...
def random_hyperparams():
    ...
    weight_decay = random.choice([1e-2, 5e-2])
    batch_size = random.choice([32, 64])
    return lr, weight_decay, batch_size

def main():
    model_name = "ConvNeXt"
    dataset_root =  '甲骨文拓片转手写+甲骨文手写体'
    DateFrom =  '甲骨文拓片转手写+甲骨文手写体'
    ...
    with open(f'{model_name}_{DateFrom}_kfold_results.csv', 'w', newline='') as csvfile:
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(all_image_paths, all_labels)):
            ...
            net.classifier[2] = nn.Linear(in_features=768, out_features=num_classes)
            net.to(device)
