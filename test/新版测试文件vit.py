...
def random_hyperparams():
    ...
    weight_decay = random.choice([1e-3, 5e-3])
...
def main():
    ...
    with open(f'{model_name}_{DateFrom}_kfold_results.csv', 'w', newline='') as csvfile:
        ...
        for fold, (train_idx, val_idx) in enumerate(skf.split(all_image_paths, all_labels)):
            ...
            # net.head = nn.Linear(in_features=768, out_features=num_classes)
            net = vit_b_16(weights=None)
            net.heads.head = nn.Linear(in_features=768, out_features=num_classes)
            if torch.cuda.device_count() > 1:
                print(f"Using {torch.cuda.device_count()} GPUs for DataParallel.")
                device_ids = [4]  # 根据实际GPU数量调整
                net.to(f'cuda:{device_ids[0]}')  # 模型放到主卡（device_ids[0]）
                net = nn.DataParallel(net, device_ids=device_ids)
            else:
                net.to(device)  # 单GPU模式才执行这个
