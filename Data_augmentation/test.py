...
#     # 对图像进行仿射变换
#     transformed_image = cv2.warpAffine(image, matrix, (cols, rows), borderValue=255)
...
def apply_data_augmentation(image):
    """应用所有的增广方法"""
    # if random.random() < 0.5:  # 提高水平翻转概率
    # image = cv2.flip(image, 1)
    # if random.random() < 0.5:  # 提高旋转概率
    #     image = random_rotation(image)
    # if random.random() < 0.8:  # 提高仿射变换概率
    image = random_affine_transform(image)
    # # if random.random() < 0.3:  # 提高椒盐噪声概率
    # image = add_salt_and_pepper_noise(image, 0.03)
    # # if random.random() < 0.3:  # 提高高斯噪声概率
    # image = add_gaussian_noise(image)
    # if random.random() < 0.7:  # 提高明暗变化概率
    #     image = adjust_brightness_contrast(image, random.uniform(0.8, 1.2), random.randint(-20, 20))
    # if random.random() < 0.5:  # 提高灰度处理概率
    #     image = random_grayscale(image)
    # if random.random() < 0.3:  # 提高高斯模糊概率
...
if __name__ == "__main__":
    ...
    parser.add_argument('--input_dir', type=str, default='test', help="Path to the input dataset directory")
    parser.add_argument('--output_dir', type=str, default='test_aug', help="Path to the output augmented dataset directory")
